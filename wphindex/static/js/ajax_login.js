...
$(function() {
	...
	$('.J_mobile_reg_button').click(function(event) {
		// 首先获取账户，用户输入的
	    	...
	    	$.ajax({
	    	    ...
	    	    success: function(dt) {
	    	    	...
	    	    	switch(dt){
	    	    		...
	    	    		case '1': 
	    	    		$('form span').hide();
	    	    		$('#user-name').parent().next().children().show();
	    	    		break;
	    	    		// “2”表示密码错误
	    	    		case '2': 
	    	    		$('form span').hide();
	    	    		$('#user-password').parent().next().children().show();
	    	    		break;
	    	    		// “3”表示成功登录，执行跳转，跳转到后台页面admin.php
	    	    		case '3': window.location.href = 'admin.php';
	    	    	}
	    	    }
	    	});
	    	// $.ajax（）方法结束
